...
class TeradataSqlGenerator() : JdbcSqlGenerator(namingTransformer = StandardNameTransformer()) {
    ...
    override fun cdcDeletedAtNotNullCondition(): Condition {
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - cdcDeletedAtNotNullCondition - temp - {}",
            temp
        )
        return temp
    }
    ...
    override fun extractRawDataFields(
        ...
        columns.forEach { (key, value) ->
            LOGGER.info(
                "Satish - TeradataSqlGenerator - extractRawDataFields - key - {} - value - {}",
                key,
                value
            )
            if (
                ...
                fields.add(
                    field(
                            ((((((("case when (REGEXP_SIMILAR(" + name(COLUMN_NAME_DATA)) +
                                ".JSONExtractValue('$." +
                                field(
                                    key.originalName,
                                )) +
                ...
                fields.add(
                    field(
                            ((((((((("case when " + "cast(" + name(COLUMN_NAME_DATA)) +
                                ".JSONExtract('$." +
                                field(
                                    key.originalName,
                                )) +
                                ...
                                "') as " +
                                toDialectType(value) +
                                ") FROM 2 FOR LENGTH (cast(" +
                                name(
                                    COLUMN_NAME_DATA,
                                )) + ".JSONExtract('$." + field(key.originalName)) +
                                "') as " +
                                toDialectType(
                                    value,
                                ) +
    ...
    override fun getRowNumber(
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - getRowNumber - primaryKeyFields - {}",
            primaryKeyFields
        )
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - getRowNumber - orderedFields - {}",
            orderedFields
        )
        val query =
            rowNumber()
    ...
    override fun toDialectType(airbyteProtocolType: AirbyteProtocolType): DataType<*> {
        LOGGER.info(
            "Satish - TeradataSqlGenerator - toDialectType - airbyteProtocolType - {} ",
            airbyteProtocolType
        )
        val s =
    ...
    override fun createTable(stream: StreamConfig, suffix: String, force: Boolean): Sql {
        LOGGER.info(
            "TeradataSqlGenerator - CreateTable - tablename - {}, force? - {}",
            stream.id.finalName,
            force
        )
        ...
        LOGGER.info(
            "TeradataSqlGenerator - CreateTable - finalTableIdentifier- {}",
    ...
    override fun createTableSql(
        ...
        val createTableSql =
            dsl.createTable(name(namespace, tableName))
                ...
                .sql
        LOGGER.info(
            "Satish - TeradataSqlGenerator - CreateTableSql - CreateTableSQL: {}",
            createTableSql
        )
    ...
    override fun insertAndDeleteTransaction(
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - rawTableRowsWithCast: {}",
            rawTableRowsWithCast
        )
        val finalTableFields =
            buildFinalTableFields(
                streamConfig.columns,
                getFinalTableMetaColumns(true),
            )
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - finalTableFields: {}",
            finalTableFields
        )
        val rowNumber =
            getRowNumber(
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - rowNumber: {}",
            rowNumber
        )
        val filteredRows =
            name(NUMBERED_ROWS_CTE_ALIAS)
                .`as`(
                    DSL.select(finalTableFields).select(rowNumber).from(rawTableRowsWithCast),
                )
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - filteredRows: {}",
            filteredRows
        )
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - insertStmtWithDedupe: {}",
        ...
        val insertStmt =
            insertIntoFinalTable(
                ...
                .getSQL(ParamType.INLINED)
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - insertStmt: {}",
            insertStmt
        )
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - deleteStmt: {}",
            deleteStmt
        )
        ...
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - deleteCdcDeletesStmt: {}",
            deleteCdcDeletesStmt
        )
        val checkpointStmt = checkpointRawTable(rawSchema, rawTable, minRawTimestamp)
        LOGGER.info(
            "Satish - TeradataSqlGenerator - insertAndDeleteTransaction - checkpointStmt: {}",
            checkpointStmt
        )
